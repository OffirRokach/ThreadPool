...
class WaitablePQueueCVTest {
    ...
    void setUp() throws InterruptedException {
        Comparator<Integer> integerComparator = Comparator.comparingInt(num -> num);
    ...
    void enqueue() throws InterruptedException {
        ...
        for(int i = 0; i < NUM_OF_PRODUCERS; ++i){
            ...
            enqueueThreads[i] = new Thread(()->{
                for(int j = baseValue; j < WORK_PER_PRODUCER * (finalI + 1); ++ j){
                    queue.enqueue(baseValue + j);
                }
            });
